...
function Setting() {
		...
		if(isPost(req)){
			...
			dataTableConfig(req,res,null,function(configDataTable){
				asyncParallel([
					(callback)=>{
						/** Get list of setting **/
						collection.find(configDataTable.search_condition,{projection: {_id:1,title:1,value:1,modified:1,key_value:1}}).sort(configDataTable.sort_condition).limit(limit).skip(skip).toArray((err,result)=>{
							callback(err, result);
						});
					},
					(callback)=>{
					...
					(callback)=>{
						...
						collection.countDocuments(configDataTable.search_condition,(err,filterContResult)=>{
	 ...
	 * @return render/json
	 */
   	this.postAddSetting = (req, res)=>{
			const title 		= 	(req.body.title)			? 	req.body.title			:"";
			const value 		= 	(req.body.value)			?	req.body.value			:"";
			const keyValue 	= 	(req.body.key_value)		? 	req.body.key_value		:"";
			const inputType 	= 	(req.body.input_type)		? 	req.body.input_type		:"";
			const validatType = 	(req.body.validate_type)	? 	req.body.validate_type	:"";
			var   order 		= 	(req.body.order)			? 	req.body.order			:"";
			const editable 	= 	(req.body.editable)			? 	req.body.editable		:0;
			const required 	= 	(req.body.required)			? 	req.body.required		:0;
			// const validRegx	= 	/^[0-9]+$/;
			...
			// var errors = uniqueValidations(req.validationErrors(),req);
										...
										if(!error) {
											...
											writeSettingDetails(req, res).then(response=>{
												...
												res.send({
													...
													rediect_url: 	WEBSITE_ADMIN_URL+"settings",
											...
											res.send({
												status : STATUS_ERROR,
												message: [{param:STATUS_ERROR,msg:res.__("admin.system.something_going_wrong_please_try_again ")}],
											});
										}
									}
								);
				...
				req.body.settings.forEach((records,index)=>{
					...
					if(value =="" && required == REQUIRED && inputType !="checkbox"){
						...
						if(!validRegx.test(value)){
							...
							errors.push({"param":"setting_"+index+"_value","msg":res.__("admin.setting.it_should_be_in_between_1_to_99") + " " +title});
						}
					}
													...
													if(!err){
														if(req.body.settings.length-1 == dataIndex){
															...
															writeSettingDetails(req, res).then(response=>{
																...
																res.send({
																	status		: STATUS_SUCCESS,
																	rediect_url : WEBSITE_ADMIN_URL+"settings/prefix/"+type,
